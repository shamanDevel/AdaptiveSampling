...
def benchmark():
    DEBUG = False
    IMAGE_EXPORT = []#[(2**9, 1024)] # screen, volume resolution
    ...
    NETWORK = ("network", NETWORK_DIR+"enhance4_imp100+N_epoch500")
    ...
    OUTPUT_FILE = "../result-stats/IsoBenchmark.tsv"
    ...
    if 0:
        # EJECTA
        VOLUME = "snapshot_050_256.cvol"
        STEPSIZE = 0.125
        POSITION_SAMPLER = lambda: (1.1**0.3) * randomPointOnSphere()
        SETTINGS = "Iso-Ejecta-settings.json"
        UPSCALING = 8
                ...
                for resolution in SCREEN_RESOLUTIONS:
                    ...
                    for i in trange(NUM_SAMPLES, desc='Samples', file=orig_stdout, dynamic_ncols=True, leave=True):
                        ...
                        if (resolution, volumeResolution) in IMAGE_EXPORT:
                            filename = "export_%d_%d_%d.png"%(
                                resolution, volumeResolution, i)
                            image = high_res[1:4,:,:]*0.5+0.5
                        ...
                        # reconstruction
                        reconstruction_input = reconstruction_input[:7,:,:].unsqueeze(0)
                        previous_input = torch.zeros(
                                1,6,
